...
selected_knock_T_5 = p + which(abs(beta_hat[seq(p + 1, ncol(XK))]) > eps)
...
selected_knock_T_10 = p + which(abs(beta_hat[seq(p + 1, ncol(XK))]) > eps)
...
# Simulations
In order to validate our observations from the illustrative example above, we conduct Monte Carlo simulations and plot the resulting averaged FDP and TPP over the number of included knockoffs T. Note that the averaged FDP and TPP are estimates of the FDR and TPR, respectively.
...
actives = which(beta > 0) # indices of true active variables
L = p # number of knockoffs
T_vec = c(1, 2, 5, 10, 20, 50, 100) # stopping points, i.e, number of included knockoffs before terminating the solution path
...
for(t in seq_along(T_vec)){
  for(mc in seq(MC)){
    ...
    # Generate knockoff matrix and append it to X
    knocks = matrix(stats::rnorm(n * p), nrow = n, ncol = p)
    XK = cbind(X, knocks)
    
    # Create tlarsCpp object
    mod_tlars = tlars_model(X = XK, y = y, num_knocks = L, type = 'lar', info = FALSE)
...
# Compute estimates of FDP and TPP by averaging FDP and TPP over MC Monte Carlo runs
FDR = colMeans(FDP)
TPR = colMeans(TPP)
```
